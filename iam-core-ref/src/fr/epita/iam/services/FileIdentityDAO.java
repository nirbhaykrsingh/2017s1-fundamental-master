...
public class FileIdentityDAO implements IdentityDAO {
	...
	public FileIdentityDAO() {
		...
		File file = new File("c:/java/temp/tests/identities.txt");
	...
	public List<Identity> search(Identity criteria) {
		...
		while (this.scanner.hasNext()) {
			...
			Identity identity = new Identity(displayName, uid, email,null);
		...
		this.scanner.close();
		try {
			this.scanner = new Scanner(new File("C:/java/temp/tests/identities.txt"));
	...
	public void update(Identity identity) throws DaoUpdateException {
		...
		while (this.scanner.hasNext()) {
			...
			Identity currentIdentity = new Identity(displayName, uid, email,null);
			// identity found
		...
		// 1 . file initialization
		File file = new File("c:/java/temp/tests/identities.tmp.txt");
		FileWriter writer = null;
		FileReader reader = null;
		if (!file.exists()) {
			// create the directory structure
		...
		File oldFile = new File("c:/java/temp/tests/identities.txt");
		Files.delete(oldFile.toPath());
		Files.move(file.toPath(), oldFile.toPath(), null);

		// TODO make the initialization again
		// 1 . file initialization
		oldFile = new File("c:/java/temp/tests/identities.txt");
		writer = null;
		reader = null;
		if (!file.exists()) {
			// create the directory structure
